...
function removeFile(path) {
	if (confirm("确认删除这个文件吗?\n" + path)) {
		$.get($('#admin').val() + '?aa=removeFile&path=' + path, function(msg) {
			getDir($('#selPath').val());
		});
	}
}
...
function removeDir(path) {
	if (confirm("确认删除这个目录吗?\n" + path)) {
		$.get($('#admin').val() + '?aa=removeDir&path=' + path, function(msg) {
			getDir($('#selPath').val());
		});
	}
}
...
function getFile(path) {
	...
	var url = $('#admin').val() + '?aa=getFile&path=' + path;
...
function saveFile() {
	...
	$.post($('#admin').val() + '?aa=saveFile', data, function(msg) {
		// alert(msg);
		$('#' + selFile).css('color', '');
	});
}
...
function newFile() {
	...
	if (fileName) {
		var url = $('#admin').val() + '?aa=newFile&path=' + fileName;
...
function newDir() {
	...
	if (dirName) {
		var url = $('#admin').val() + '?aa=newDir&path=' + dirName;
		$.get(url, function(msg) {
			if (msg != 'ok') {
				alert(msg);
			} else {
...
function moveFile(oldName) {
	...
	if (newName) {
		var url = $('#admin').val() + '?aa=moveFile&path=' + oldName
				+ '&newPath=' + newName;
		$.get(url, function(msg) {
			if (msg != 'ok') {
				alert(msg);
...
function moveDir(oldName) {
	...
	if (newName) {
		var url = $('#admin').val() + '?aa=moveDir&path=' + oldName
				+ '&newPath=' + newName;
		$.get(url, function(msg) {
			if (msg != 'ok') {
				alert(msg);
...
function upload() {
	$.ajaxFileUpload({
		url : $('#admin').val() + '?aa=uploadFile&path=' + $('#selPath').val(),
		secureuri : false,
		fileElementId : 'file',
		dataType : 'json',
		data : {
...
function zip(zip) {
    ...
    if (to) {
    	url = $('#admin').val() + '?aa=zipextract&zip=' + zip + '&to=' + to;
        $.get(url, function(msg) {
			if (msg != 'ok') {
				alert(msg);
			} else {
